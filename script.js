...
// Не сделал изменение данных "about", потому что изначально замыслил показать полный тескт при нажатии

// Не сделал сортировку для телефона, потому что мне непонятно для чего и по какому принципу можно сортировать телефон

const changeDataTd = () => {
    let td = document.querySelectorAll('td') // Нахожу все теги "td"
    td.forEach((tdChange) => { // Перебор "td"
        tdChange.addEventListener('click', function () {
            if (tdChange.children.length > 0) {
                // Нельзя изменить колонку "about"
            }
            else { // 
                let input = document.createElement('input') // Создаю новый элемент input
                input.value = tdChange.innerText // Присваиваю новое значение input(-у)
                tdChange.innerText = '' // Стираю значение у тега td
                tdChange.appendChild(input) // Добавляю input
                input.focus() // Фокус на элементу input
                input.addEventListener('blur', function () { // Накладываю событие 'blur', которое сработает при потере фокуса
                    tdChange.innerText = input.value
                })
            }
        })
    })
...
const symbolString = 60

let dataP = [] // Создаю новый массив, в который буду вносить данные из св-ва "about", которые скрываю

renderPage(JSON) // Вызываю функцию, которая принимает массив "JSON" и отрисовывает таблицу

changeDataTd()

function renderPage(AAA) {
    for (let i = 0; i < AAA.length; i++) { // Прохожусь по циклу
        let table = document.querySelector('#sort') // Нахожу всю таблицу по ID
        let tbody = table.querySelector('tbody') // Нахожу в таблице тег tbody, в который буду вносить данные из массива "JSON"
        // Вставляю разметку в тег tbody перед закрывающим тегом
        tbody.insertAdjacentHTML('beforeend', `<tr id = "${AAA[i].id} ">
        <td>${AAA[i].name.firstName}</td>
        <td>${AAA[i].name.lastName}</td>
        <td>${AAA[i].phone}</td>
        <td><p class = "paragraph" id = "id${i}">${AAA[i].about}</p></td>
        <td>${AAA[i].eyeColor}</td>
        </tr>`)

        dataP.push({ // Вношу в пустой массив данные из св-ва "about", которые скрываю
            id: `id${i}`, // Присваиваю ID, чтобы по нему найти нужные данные "about"
            about: `${AAA[i].about}`
        })

        if (AAA[i].about.length > symbolString) { // Если кол-во символов в "about" больше symbolString
            let p = document.querySelector(`#id${i}`) // Нахожу необходимый тег по ID
            p.textContent = p.textContent.slice(0, symbolString) + '...' // Обрезаю, для отображения "about" в две строки
        }
    }
    let p = document.querySelectorAll('p') // Нахожу все теги p
    p.forEach(function (e) { // Перебираю теги p
        e.addEventListener('click', pClick) // Накладываю обработчик собтия 'click'. При нажатии вызовется ф-я pClick
    })

    function pClick(event) {
        if (event.target.textContent.length == 63) { // Если кол-во символов равно 63, то по ID элемента нахожу необходимые данные "about" и отображаю их полностью
            let id = event.target.id
            let pText = dataP.find(item => item.id == id)
            event.target.textContent = pText.about
        }
        else { // При втором нажатии на область тега "p", текст обрежется в две строки
            event.target.textContent = event.target.textContent.slice(0, symbolString) + '...'
        }
    }
    changeDataTd()
}

let th = document.querySelectorAll('th') // Нахожу все теги th
th.forEach(function (thSort) { // Перебираю th
    let table = document.querySelector('#sort') // Нахожу всю таблицу по ID
    let butSort = thSort.querySelector('.sortButton') // Нахожу все элементы по классу sortButton
    if (butSort == null) {

    } else
        butSort.addEventListener('click', function () { // Накладываю обработчик собтия 'click' на кнопку 'Сортировка'
            let copyJSON = [...JSON] // Создаю копию массива "JSON", чтобы не изменять главный
            switch (butSort.parentNode.id) { // Проверяю по ID кнопки
                case "001": {
                    copyJSON.sort((a, b) => { // 
                        if (a.name.firstName > b.name.firstName) {
                            return 1 // Если по алфавиту идет позже
                        }
                        else if (a.name.firstName < b.name.firstName) {
                            return -1 // Если по алфавиту идет раньше
                        }
                    })
                    dataP = [] // Присвоил массиву пустое значение, чтобы записать новые данные после перерисовки таблицы
                    break // Закончил проверку
                }
                case "002": { // По аналогии case "001"
                    copyJSON.sort((a, b) => {
            ...
            let tbody = table.querySelector('tbody') // Нахожу полностью тег tbody
            tbody.innerHTML = ''; // Стираю всю таблицу за исключение заголовка
            renderPage(copyJSON) // Перерисовываю таблицу с новым массивом "copyJSON"
        })
    let butReturn = thSort.querySelector('.sortReturn') // Нахожу кнопку "Отмена сортировки"
    if (butReturn == null) {

    } else
        butReturn.addEventListener('click', function () { // Накладываю обработчик собтия 'click' на кнопку 'Отмена сортировки'
            let table = document.querySelector('#sort')
            let tbody = table.querySelector('tbody')
            tbody.innerHTML = ''; // Стираю всю таблицу за исключение заголовка
            renderPage(JSON) // Перерисовываю таблицу с изначальным массивом "JSON"
        })
})